...
+ poll.frequency `5000`
+ poll.delay `0`
+ watermark.default.expression `#[groovy: new java.util.Date()]`
+ msdyn.offset `6000`

**Note**: msdyn.offset is used while polling MS Dynamics for changes. The query time bound might need to be decreased, depending on the your setup, this value in order to catch all data modifications.
...
1. Before the Input stage the Anypoint Template will query all the existing accounts from MS Dynamics modified after watermark.
